...
function DeployScreen({
  ...
  // Config state
  const [mode, setMode] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(defaultMode === 'full' ? 'full' : 'code');
  const [checked, setChecked] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)([...DEFAULT_CHECKED]);
  const [dbConfirmed, setDbConfirmed] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)(true);
  ...
  if (phase === 'deploying' || phase === 'done') {
    ...
    }, result.success ? mode === 'full' ? `Full deploy complete — ${(result.data?.synced || []).length} directories synced.` : `Code deploy complete — synced: ${(result.data?.targets || []).join(', ')}` : isCancelled ? 'Deploy cancelled. Your site was not changed.' : `Deploy failed: ${result.error}`), running && /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
  ...
  })), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(ArchiveFormatPicker, {
    value: archiveFormat,
