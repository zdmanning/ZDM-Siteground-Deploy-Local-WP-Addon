...
const settingsStore = __webpack_require__(/*! ./services/settings-store */ "./src/main/services/settings-store.js");
const exportService = __webpack_require__(/*! ./services/export-service */ "./src/main/services/export-service.js");
...
module.exports = function (context) {
  ...
  ipcMain.handle('sgd:keys:rotate', async (_e, profileId, newKeyId) => {
    ...
    if (oldKeyId && oldKeyId !== newKeyId) {
      ...
      const profilesResult = profileStore.getProfiles();
      const stillInUse = profilesResult.success ? profilesResult.data.some(p => p.keyId === oldKeyId) : true; // be conservative: if we can't check, keep the key
  ...
  ipcMain.handle('sgd:fs:listDir', async (_e, profileId, relPath) => {
    ...
    if (!profile.localSiteId) return {
      success: false,
      error: 'No local site linked to this profile'
    };
    const wpContentPath = localAdapter.getSiteWpContentPath(profile.localSiteId);
    if (!wpContentPath) return {
      success: false,
      error: 'Could not resolve wp-content path'
    };
    const targetPath = relPath ? path.join(wpContentPath, ...relPath.split('/')) : wpContentPath;
...
async function importLocalDatabase(siteId, sqlPath, onLog) {
  ...
  // Try WP-CLI first
  let res = await _run(`wp db import ${_lq(sqlPath)} --path=${_lq(wpRoot)} --allow-root`, {
    cwd: wpRoot
  });
  if (res.exitCode === 0) {
  ...
  const pwArg = creds.dbPassword ? `-p"${creds.dbPassword.replace(/"/g, '')}"` : '';
  const cmd = ['mysql', `-h"${creds.dbHost}"`, `-P${creds.dbPort}`, `-u"${creds.dbUser}"`, pwArg, `"${creds.dbName}"`, `< ${_lq(sqlPath)}`].filter(Boolean).join(' ');
  res = await _run(cmd, {
    cwd: wpRoot
  });
  if (res.exitCode !== 0) {
    const summary = (res.stderr || res.stdout).trim().split('\n').slice(-3).join(' | ');
    return _err('Local database import failed. Neither WP-CLI nor mysql CLI succeeded.\n' + `mysql error: ${summary}`);
  }
...
async function runLocalSearchReplace(siteId, productionDomain, localDomain, onLog) {
  ...
  const res = await _run(`wp search-replace ${_lq(productionDomain)} ${_lq(localDomain)} ` + `--all-tables --precise --recurse-objects --skip-plugins --skip-themes ` + `--path=${_lq(wpRoot)} --allow-root`, {
    cwd: wpRoot
  });
  if (res.exitCode !== 0) {
    const summary = (res.stderr || res.stdout).trim().split('\n').slice(-3).join(' | ');
...
const settingsStore = __webpack_require__(/*! ./settings-store */ "./src/main/services/settings-store.js");
...
async function runCodeDeploy(profile, options = {}, onLog) {
  ...
  try {
    ...
    for (const item of sourceItems) {
      ...
      `mkdir -p ${_q(destParent)};`, `if command -v rsync > /dev/null 2>&1; then`, `  rsync -az --delete ${rsyncExcludes} ${_q(srcDir + '/')} ${_q(destDir + '/')};`, `else`, `  rm -rf ${_q(destDir)} && cp -rp ${_q(srcDir)} ${_q(destParent + '/')};`, `fi`].join(' ');
      let syncOut = '';
...
async function runFullDeploy(profile, options = {}, onLog) {
  ...
  try {
    ...
    for (const item of sourceItems) {
      ...
      const rsyncExcludes = excludeFolders.map(f => `--exclude=${_q(f)}`).join(' ');
      const syncCmd = [`if command -v rsync > /dev/null 2>&1; then`, `  rsync -az --delete ${rsyncExcludes} ${_q(srcDir + '/')} ${_q(destDir + '/')};`, `else`, `  rm -rf ${_q(destDir)} && cp -rp ${_q(srcDir)} ${_q(remoteWebRoot + '/wp-content/')};`, `fi`].join(' ');
      let syncOut = '';
...
async function runDbDeploy(profile, options = {}, onLog) {
  const pid = profile.id || null;
  const runId = uuidv4();
  const emit = (lvl, msg) => _emit(lvl, msg, onLog, pid, {
    runId,
    actionType: 'db_deploy',
    profileId: pid
  });
  const dbLog = entry => {
    const rich = {
  ...
  try {
    ...
    if (!importResult.success) {
      ...
      if (localTablePrefix && remoteTablePrefix && localTablePrefix !== remoteTablePrefix) {
        ...
        const prefixUpdateResult = await dbSvc.updateRemoteTablePrefix(sshConn, remoteWebRoot, localTablePrefix, dbLog);
        if (!prefixUpdateResult.success) {
          return _err(`Database imported, but remote wp-config prefix update failed.\n` + `${prefixUpdateResult.error}\n\nRemote DB backup: ${dbBackupPath}`);
        }
      }
...
async function runCodePull(profile, options = {}, onLog) {
  ...
  try {
    ...
    let mkdirOut = '';
    await sshConn.exec(`mkdir -p ${_q(remoteTempDir)}`, c => {
      mkdirOut += c;
    });
    ...
    for (const target of requestedTargets) {
      ...
      let existOut = '';
      await sshConn.exec(`[ -d ${_q(remotePath)} ] && echo yes || echo no`, c => {
        existOut += c;
      });
    ...
    let zipOut = '';
    const zipResult = await Promise.race([sshConn.exec(`cd ${_q(remoteWpContent)} && zip -r ${_q(remoteZipPath)} ${zipArgs} 2>&1`, c => {
      zipOut += c;
    }), token.cancelPromise]);
    ...
    let sizeOut = '';
    await sshConn.exec(`stat -c %s ${_q(remoteZipPath)} 2>/dev/null || echo 0`, c => {
      sizeOut += c;
    });
    ...
    fs.mkdirSync(path.dirname(localTempZip), {
      recursive: true
    });
    emit('info', `Downloading from ${profile.sshHost}…`);
    let lastPct = -1;
    const dlResult = await Promise.race([sftpSvc.downloadFile(profile, remoteZipPath, localTempZip, ({
      percent
    }) => {
      const band = Math.floor(percent / 25) * 25;
      if (band > lastPct) {
    ...
    _outcome = 'success';
    _outcomeMeta = {
      targets: remoteTargets.map(t => t.subDir)
    };
    return _ok({
      runId,
      targets: remoteTargets.map(t => t.subDir)
...
async function runDbPull(profile, options = {}, onLog) {
  ...
  try {
    ...
    const localBackupResult = await dbSvc.exportLocalDatabase(profile.localSiteId, bk.filePath, dbLog);
    ...
    let exportOut = '';
    let exportRes = await Promise.race([sshConn.exec(`wp db export ${_q(remoteSqlPath)} --path=${_q(remoteWebRootVal)} --allow-root 2>&1`, c => {
      exportOut += c;
    }), token.cancelPromise]);
    if (exportRes.exitCode !== 0) {
      ...
      exportOut = '';
      exportRes = await Promise.race([sshConn.exec(
      // Use the same approach as backupRemoteDatabase — read creds from wp-config via cat+parse
      `eval $(php -r "` + `\\$c=file_get_contents('${remoteWebRootVal}/wp-config.php');` + `preg_match(\"/define\\\\s*\\\\(\\\\s*\\\\'DB_NAME\\\\'\\\\s*,\\\\s*\\\\'([^\\\\']+)/\",\\$c,\\$m); echo 'DB_NAME='.\\\$m[1].'\\n';` + `preg_match(\"/define\\\\s*\\\\(\\\\s*\\\\'DB_USER\\\\'\\\\s*,\\\\s*\\\\'([^\\\\']+)/\",\\$c,\\$m); echo 'DB_USER='.\\\$m[1].'\\n';` + `preg_match(\"/define\\\\s*\\\\(\\\\s*\\\\'DB_PASSWORD\\\\'\\\\s*,\\\\s*\\\\'([^\\\\']*)/\",\\$c,\\$m); echo 'DB_PASSWORD='.\\\$m[1].'\\n';` + `preg_match(\"/define\\\\s*\\\\(\\\\s*\\\\'DB_HOST\\\\'\\\\s*,\\\\s*\\\\'([^\\\\']+)/\",\\$c,\\$m); echo 'DB_HOST='.\\\$m[1].'\\n';` + `") && MYSQL_PWD="$DB_PASSWORD" mysqldump -h"$DB_HOST" -u"$DB_USER" --single-transaction --routines --triggers --no-tablespaces "$DB_NAME" > ${_q(remoteSqlPath)} 2>&1`, c => {
        exportOut += c;
      }), token.cancelPromise]);
      if (exportRes.exitCode !== 0) {
    ...
    // Verify size
    let szOut = '';
    await sshConn.exec(`stat -c %s ${_q(remoteSqlPath)} 2>/dev/null || echo 0`, c => {
      szOut += c;
    });
    ...
    fs.mkdirSync(path.dirname(localTempSql), {
      recursive: true
    });
    emit('info', `Downloading SQL from ${profile.sshHost}…`);
    let lastPct = -1;
    const dlResult = await Promise.race([sftpSvc.downloadFile(profile, remoteSqlPath, localTempSql, ({
      percent
    }) => {
      const band = Math.floor(percent / 25) * 25;
      if (band > lastPct) {
    ...
    const importResult = await dbSvc.importLocalDatabase(profile.localSiteId, localTempSql, dbLog);
    ...
    if (productionDomain && localDomain) {
      await dbSvc.runLocalSearchReplace(profile.localSiteId, productionDomain, localDomain, dbLog);
    } else {
...
const FILE_FILTERS = [{
  name: 'SiteGround Deploy Export',
  extensions: ['sgdexport']
}];
...
function _privPath(keyId) {
  return path.join(_keyDir(), keyId);
}
function _pubPath(keyId) {
  return path.join(_keyDir(), `${keyId}.pub`);
}
...
async function exportProfiles(profileIds) {
  ...
  const toExport = Array.isArray(profileIds) && profileIds.length ? allRes.data.filter(p => profileIds.includes(p.id)) : allRes.data;
...
async function pickAndParseImport() {
  ...
  if (!bundle || bundle.version !== EXPORT_VERSION || !Array.isArray(bundle.profiles) || bundle.profiles.length === 0) {
...
async function applyImport(profiles, decisions = {}) {
  const existingRes = profileStore.getProfiles();
  const existingMap = new Map(existingRes.success ? existingRes.data.map(p => [p.id, p]) : []);
  let imported = 0,
    skipped = 0,
    overwritten = 0,
    renamed = 0;
  for (const profile of profiles) {
    ...
    if (decision === 'overwrite') {
      ...
      if (_privateKey) {
        fs.writeFileSync(privPath, _privateKey, {
          encoding: 'utf8',
          mode: 0o600
        });
      }
    ...
    const importRes = profileStore.createProfile({
      ...rest,
      keyId: useKeyId
    });
...
function rotateProfileKey(profileId, newKeyId) {
  ...
  if (!existsResult.data.exists) {
    return _err('New key pair files not found on disk. ' + 'Generate the key pair before rotating.');
  }
  const profiles = _readAll();
...
async function downloadFile(profile, remotePath, localPath, onProgress, cancelToken) {
  const privateKeyPath = keyManager.getPrivateKeyPath(profile.keyId);
  if (!privateKeyPath) {
    return _err('SSH private key not found. Open the profile and regenerate the key.');
  }
  let privateKey;
  try {
    privateKey = await fs.promises.readFile(privateKeyPath, 'utf8');
