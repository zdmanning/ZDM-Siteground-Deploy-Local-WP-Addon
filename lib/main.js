...
async function runCodeDeploy(profile, options = {}, onLog) {
  ...
  try {
    ...
    for (const item of sourceItems) {
      ...
      `mkdir -p ${_q(destParent)};`, `if command -v rsync > /dev/null 2>&1; then`, `  rsync -az --delete --exclude='.git' ${_q(srcDir + '/')} ${_q(destDir + '/')};`, `else`, `  rm -rf ${_q(destDir)} && cp -rp ${_q(srcDir)} ${_q(destParent + '/')};`, `fi`].join(' ');
      let syncOut = '';
